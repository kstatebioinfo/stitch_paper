...
Genome assembly and scaffolding algorithms are inherently limited by the length of the nucleic acid used as starting material to generate data. Data can only be used to traverse repeat, polymorphic or low complexity regions of the genome if it includes information gathered from one molecule about both portions of genome flanking the difficult region. The latest release of PacBio chemistry P6-C4 has an N50 of 14 kb with a maximum length of 40 kb (HOW TO CITE?). Illumina Long Distance Jump Libraries can also span 40 kb (HOW TO CITE?). MinION nanopore sequence reads have an average read length of $< 7$ kb \cite{Nanopore2014}. Illumina TruSeq synthetic long-reads can span up to 18.5 kb however they fail to assemble if the sequence has problem regions longer than the component reads used to assemble the synthetic reads (e.g. in the herterochromatin) \cite{Moleculo2014}. The OpGen Argus platform outputs optical maps that have an average length of 150 kb to 2 Mb for up to 13 Gb per run (HOW TO CITE?). The Irys platform from BioNano Genomics outputs optical maps that have an average length of (BIONANO FILL IN) for up to (BIONANO FILL IN) per run (HOW TO CITE?).
