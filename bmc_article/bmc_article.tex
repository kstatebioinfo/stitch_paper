...
%%
\section*{Background}
In this article we present software that leverages genome maps assembled from ultra-long single molecule maps to improve the contiguity of NGS assemblies. We report the results of applying these tools to the \textit{Tribolium castaneum} genome.
...
\subsubsection*{Super scaffolding: Flagging Potential Misassemblies}

This algorithm is meant to be an intermediate refinement of draft genomes prior to further finer scale refinement at the sequence level. 
...
\section*{Figures}
  \begin{figure}[h!]
  	\caption{\csentence{Data analysis steps.}
  		(1) Autonoise converts TIFF images of molecules to (2) BNX text files. (3) NGS scaffolds are \textit{in silico} labeled with fa2cmap\_multi producing (4) a \textit{in silico} genome map. (5) AssembleIryscluster uses \textit{in silico} genome maps, BNX files and estimated genome size to (6) adjust molecule stretch and set assembly parameters. (7) Assembler produces (8) a BioNano genome map. (9) RefAligner aligns the BioNano genome map to the \textit{in silico} genome map producing (10) an XMAP. (11) XMAP, \textit{in silico} genome map and BioNano genome map are used by stitch to produce super scaffolded (stitched) NGS scaffolds. (13) Until no more super scaffolds are created the stitched NGS scaffolds are \textit{in silico} labeled with fa2cmap\_multi producing (14) a CMAP that is aligned to (9) the BioNano genome map and steps 10-15 are iterated.}
  \end{figure}
  \begin{figure}[h!]
...
\begin{table}[h!]
	...
	\begin{tabular}{cccc}
		\hline
		Chromosome linkage group (ChLG) & Tcas5.0 NGS scaffolds & Unplaced NGS scaffolds added in Tcas5.2 & Tcas5.2 NGS super scaffolds \\ \hline
		X & 13 & +2 & 2\\
		2 & 18 & +1 & 10\\
		3 & 29 & +4 & 20\\
		4 & 6 & +2 & 2\\
