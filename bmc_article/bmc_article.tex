...
We introduced new tools to facilitate BioNano molecule map assembly optimization and genome finishing steps with the resultant BioNano genome maps. These tools were validated using the medium-sized (200 Mb) beetle genome from \textit{T. castaneum}. The \textit{T. castaneum} genome was originally assembled using the gold-standard Sanger assembly strategy and had the benefit of extensive genetic maps and LongDistance Illumina Jump libraries. Despite this we were able to more than than triple the genome N50 using the optimal BioNano genome map and Stitch. Demonstrating that both the AssembleIrysCluster method of optimization and Stitch can be used to improve the contiguity of an draft genome.

As the variety of genome assembly projects increases tools that are appropriate for all projects (e.g. all genome sizes, assemblies of high or low quality, all taxonomic groups, etc.) may not exist. Indeed, the results of Assemblation2 indicate that no one suite of datatypes or assembly workflow may be sufficient to best assemble even the subcategory vertebrate genomes \cite{Assem22013}. Here we have released two primary tools and many secondary scripts to summarize and work with these new data formats. However, we expect that more variety of bioinformatics tools will be developed for ultra long, single molecule map data as many more applications for these maps remain to be explored.

Since completion of the \textit{T. castaneum} genome update we have released updates for both Stitch and AssembleIrysCluster. Stitch (version 1.4.5) now allows the user to set a minimum negative gap length filter for alignments. In the event that two \textit{in silico} genome maps have a negative gap length smaller than this value stitch will automatically exclude both genome maps from consideration when super scaffolding. This new feature allows users to further customize the stringency of the Stitch output. Updates to the BioNano Assembler now handle strecth adjustment in a new fashion. This combined with the fact that assemblies will now be produce on a XeonPhi ?? necessitated updating of our assembly workflow. Because of the large number of subtle changes this required support for AssembleIrysCluster has been discontinued and AssembleIrysXeonPhi has been created in its place. AssembleIrysXeonPhi maintains all of the functionality of AssembleIrysCluster (e.g. adjusting stretch by scan and writing assembly scripts with all combinations of three $p\textnormal{-}Value Thresholds$ and three  $Minlen$ parameters.
